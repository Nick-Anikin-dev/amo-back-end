...
export const transformAmoLeadResponses = (
    ...
    const contactsMap = contacts._embedded.contacts.reduce((map, contact)=>{
        if(!map[contact.id]){
            map[contact.id] = {
                id: contact.id,
                name: contact.name,
                customFieldsValues: contact.custom_fields_values.map(customField=>({
                    name: customField.field_name,
                    values: customField.values.map(customFieldValue=>customFieldValue.value),
                })) || null,
            }
        }
        return map;
    ...
    return leads._embedded.leads.map(lead=>{ 
        return {
            ...
            contacts: lead._embedded.contacts.map(contact=>contactsMap[contact.id])
        }
    })
}